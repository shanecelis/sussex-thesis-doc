...
makeEulerStepper[eqns_, vars_, var_] := Module[{dotvars, sol, justvars, body, n, newvars, a, newvarsrules, i},
...
body = dotvars /. sol ;
(*Compile[Evaluate[Map[{#, _Real}&, newvars]], Evaluate[body]]*)
(*Compile[{{b, _Real, 1}},*)
...
makeEulerStepperC[eqns_, vars_, var_] := Module[{},
Quiet[Compile[{{a, _Real, 1}},
Evaluate[makeEulerStepper[eqns, vars, var][a]]
], {Part::partd}]]


makeEulerSolver[eqns_, vars_,var_, h_] := Module[{stepper, solver, deqns, doteqns},
...
makeEulerSolverWithConstants[eqns_, vars_,var_, h_, opts: OptionsPattern[]] := Module[{stepper, solver, deqns, doteqns, proc},
proc = OptionValue[postProcess];
deqns = makeEulerStepper[eqns,vars, var];
(*stepper = Function[{a}, a + h doteqns[a]] /. doteqns -> deqns /. h -> 0.2;*)
stepper = Quiet[Function[{a}, Evaluate[proc[Table[a[[i]], {i,Length[vars] + 1}] + h deqns[a]]]] , {Part::partd}];
(*solver = Quiet[Compile[{{s, _Real, 1},{n, _Integer}, {c,_Real, 1}}, *)

solver = Quiet[Function[{s, n, c},(*Evaluate[Nest[stepper, s, Ceiling[(max - min)/h]]]] // ReleaseHold;*)
...
ClearAll[makeEulerSolverWithConstantsC]
...
Module[{fun},
fun = OptionValue[externalFunctions];
Quiet[Compile[{{s, _Real, 1},{n, _Real}, {c2,_Real, 1}},
Evaluate[makeEulerSolverWithConstants[eqns,vars,var,h, opts][s,n,c2]]
, 
CompilationOptions -> {"InlineExternalDefinitions" -> True, "InlineCompiledFunctions" -> Automatic, "ExpressionOptimization" -> False},
...
substituteRules[vars_, v_] := Module[{},
...
makeFrogSolver[] := Module[{peqns, pvars, ceqns, cvars, ctrnn, eqns, ctrnnLength, target},
...
makeCTRNNSolver[] := Module[{peqns, pvars, ceqns, cvars, ctrnn, eqns, target, sensors, motorCoefficients, sensorCoefficients, ctrnnLength},
...
makeFrogCTRNNSolver[] := Module[{peqns, pvars, ceqns, cvars, ctrnn, eqns, target, sensors, motorCoefficients, sensorCoefficients, ctrnnLength},
...
target = {c~qpart~(geneCount + 1) , c~qpart~(geneCount + 2)};
sensors = { (* 10 + 4  = 14 *)
Norm[{u1[#], u2[#]}]&, 
u3[#]&, 
...
makeFrogMorphSolver[] := Module[{peqns, pvars, ceqns, cvars, ctrnn, eqns, target, sensors, motorCoefficients, sensorCoefficients, ctrnnLength, tailPoints, footPoints,tvars,fvars, tv,fv, tx,ty, subrules},
...
postProcessFrog[stateA_] := Module[{fixNeurons, state},
state = stateA;
...
plotConfigurationData[data_] := Module[{time,q1,q2,q3,q4,q5,q6,q7,q8, plotData, plots},
...
plotMotionData[data_] := Module[{time,q1,q2,q3,q4,q5,q6,q7,q8, plotData, plots},
...
plotNeuronData[data_] := Module[{time,q1,q2,q3,q4, q5, q6,q7, plotData, plots},
...
animateData[data_] := Module[{times, timePerFrame},
times = data[[All, 1]];
timePerFrame = (Max[times] - Min[times])/Length[times];
ListAnimate[Map[drawFrog[#[[2;;2 + 8 - 1]],r //. params, l //. params, fl //. params]&, data], 1/timePerFrame]]


showDataStats[data_] := Module[{max, min, header},
header = {t,q1,q2,q3,q4,q5,q6,q7,q8,u1,u2,u3,u4,u5,u6,u7,u8,ys1,ys2,ys3,ys4,ys5};
