...
makeRungeKuttaStepWithErrorWithConstantsC[func_] := 
    With[{func2 = funcArg},
         Compile[{{state, _Real, 1}, {h, _Real, 0}, {c, _Real, 1}},
                  Module[{k1,k2,k3,k4,k5,k6,yout,yerr},
                         k1 = h func[state, c];
                         k2 = h func[k1/5 + state, c];
                         k3 = h func[(3 k1)/40 + (9 k2)/40 + state, c];
                         k4 = h func[(3 k1)/10 - (9 k2)/10 + (6 k3)/5 + state, c];
                         k5 = h func[-((11 k1)/54) + (5 k2)/2 - (70 k3)/27 + (35 k4)/27 + 
                                  state, c];
                         k6 = h func[(1631 k1)/55296 + (175 k2)/512 + (575 k3)/13824 + (44275 k4)/110592 + (253 k5)/4096 + state, c];
...
makeRungeKuttaAdaptiveStepWithConstantsC[funcArg_, yScaleArg_] :=
    Module[{step = makeRungeKuttaStepWithErrorWithConstantsC[funcArg]},
    With[{
          yScale = yScaleArg,
          safety = 0.9,
          pGrow = -0.1,
          ...
          eps = 1*^-6,
          errCon = 1.89*^-4 (*(5/safety)^(1/pGrow)*),
          maxIncreaseFactor = 2, (* was 5 *)
          maxDecreaseFactor = 0.1},
         Compile[{{state, _Real, 1}, {hArg, _Real, 0}, {c, _Real, 1}},
                  Module[{yout = {}, yerr = {}, errmax = 0., h = 0., htemp = 0., hnext = 0., result = {{}}},
                         h = hArg;
                         While[True,
                               result = step[state, h, c];
                               yout = result[[1]];
                               yerr = result[[2]];
                               errmax = Max[Abs[yerr/yScale]];
                               errmax/eps;
                               If[errmax <= 1.0,
                                  ...
                                  htemp = safety h errmax^pShrink;
                                  h = If[h >= 0, 
                                         Max[htemp, maxDecreaseFactor h],
                                         Min[htemp, maxDecreaseFactor h]]]];
                         hnext = If[errmax > errCon,
                                    safety h errmax^pGrow,
                                    maxIncreaseFactor h];
                         yout~Join~{hnext, h}
...
makeAdaptiveIntegratorWithConstantsC[step_] :=
    With[{mystep = step},
         Compile[{{sArg, _Real, 1}, {hArg, _Real, 0}, {c, _Real, 1}, {time, _Real, 0}},
                  Module[{initTime, s, h, hdid, result},
                         s = sArg;
                         h = hArg;
                         initTime = s[[1]];
...
makeAdaptiveIntegratorWithConstantsDebugC[step_] :=
    With[{mystep = step},
         Compile[{{sArg, _Real, 1}, {hArg, _Real, 0}, {c, _Real, 1}, {time, _Real, 0}},
                  Module[{initTime, s, h, hdid = 0., result},
                         s = sArg;
                         h = hArg;
                         initTime = s[[1]];
                         While[(s[[1]] - initTime) <= time,
                               ...
                               h = result[[-2]];
                               hdid = result[[-1]];
                               (*h = Min[time - s[[1]] - initTime + 0.01, h];*)
                              ];
                         s~Join~{h, hdid}
                        ],
                Evaluate[myCompileOptions]]]
...
makeRungeKuttaAdaptiveStepWithConstants2C[funcArg_, epsArg_] :=
    Module[{step = makeRungeKuttaStepWithErrorWithConstantsC[funcArg]},
    With[{
          safety = 0.9,
          pGrow = -0.2,
          ...
          eps = epsArg, (* 1*^-6 *)
          errCon = 1.89*^-4 (*(5/safety)^(1/pGrow)*),
          maxIncreaseFactor = 5, (* was 5 *)
          maxDecreaseFactor = 0.1},
         Compile[{{state, _Real, 1}, {hArg, _Real, 0}, {c, _Real, 1}},
                  Module[{yout = {}, yerr = {}, errmax = 0., h = 0., htemp = 0., hnext = 0., result = {{}}, yScale = {}},
                         h = hArg;
                         While[True,
                               result = step[state, h, c];
                               yout = result[[1]];
